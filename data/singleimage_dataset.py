...
class SingleImageDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        # Load reference images; respect grayscale settings from options/direction
        load_mode_A = 'L' if (self.opt.input_nc == 1 and self.opt.direction == 'AtoB') or (self.opt.output_nc == 1 and self.opt.direction == 'BtoA') else 'RGB'
        load_mode_B = 'L' if (self.opt.output_nc == 1 and self.opt.direction == 'AtoB') or (self.opt.input_nc == 1 and self.opt.direction == 'BtoA') else 'RGB'
        A_img = Image.open(self.A_paths[0]).convert(load_mode_A)
        B_img = Image.open(self.B_paths[0]).convert(load_mode_B)
    ...
    def __getitem__(self, index):
        ...
        if self.opt.phase == "train":
            ...
            # Build per-domain transforms with grayscale flag
            input_nc_A = self.opt.input_nc if self.opt.direction == 'AtoB' else self.opt.output_nc
            input_nc_B = self.opt.output_nc if self.opt.direction == 'AtoB' else self.opt.input_nc
            transform_A = get_transform(self.opt, params=param, method=Image.BILINEAR, grayscale=(input_nc_A == 1))
            A = transform_A(A_img)

            param = {'scale_factor': self.zoom_levels_B[index],
                     'patch_index': self.patch_indices_B[index],
                     'flip': random.random() > 0.5}
            transform_B = get_transform(self.opt, params=param, method=Image.BILINEAR, grayscale=(input_nc_B == 1))
            B = transform_B(B_img)
        else:
            input_nc_A = self.opt.input_nc if self.opt.direction == 'AtoB' else self.opt.output_nc
            input_nc_B = self.opt.output_nc if self.opt.direction == 'AtoB' else self.opt.input_nc
            transform_A = get_transform(self.opt, method=Image.BILINEAR, grayscale=(input_nc_A == 1))
            transform_B = get_transform(self.opt, method=Image.BILINEAR, grayscale=(input_nc_B == 1))
            A = transform_A(A_img)
            B = transform_B(B_img)
